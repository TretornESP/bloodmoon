...
QFLAGSEXP ?= -cpu qemu64 -machine q35 -m 512 -boot d -cdrom ./test/useless.iso -drive if=pflash,format=raw,unit=0,file=./OVMFbin/OVMF_CODE-pure-efi.fd,readonly=on -drive if=pflash,format=raw,unit=1,file=./OVMFbin/OVMF_VARS-pure-efi.fd -net none -drive file=
...
run_exp:
	$(QEMU) $(QFLAGSEXP)$(ISODIR)/$(IMG) -drive file=./test/test.img
...
# Due to eval weird behaviour
	...
	$(QEMU) -S -s $(QFLAGSEXP)$(ISODIR)/$(IMG) -cdrom ./test/cuak.iso
