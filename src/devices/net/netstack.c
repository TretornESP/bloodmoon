...
void add_arp_cache_entry(struct nic* nic, uint8_t * ip, uint8_t * mac) {
    ...
    struct arp_cache * new = (struct arp_cache*)kmalloc(sizeof(struct arp_cache));
    memcpy(new->ip, ip, 4);
                    ...
                    case ETHERT_ARP: {
                        ...
                        if (arp.oper == ARP_REQUEST) {
                            ...
                            if (memcmp(arp.tpa, c->ip, 4) == 0) {
                                ...
                                reply.sha = (uint8_t*)kmalloc(arp.hlen);
                                reply.spa = (uint8_t*)kmalloc(arp.plen);
                                reply.tha = (uint8_t*)kmalloc(arp.hlen);
                                reply.tpa = (uint8_t*)kmalloc(arp.plen);
                                ...
                                uint8_t * reply_data = (uint8_t*)kmalloc(reply_size);
...
struct nic* create_nic(const char* name, const char * device_name) {
    for (int i = 0; i < 16; i++) {
        if (nics[i] == 0) {
            nics[i] = (struct nic*)kmalloc(sizeof(struct nic));
...
volatile struct packet* create_rx_packet(struct nic * n, uint16_t len) {
    ...
    volatile struct packet * p = (struct packet*)kmalloc(sizeof(struct packet));
        

    p->len = len;
    

    p->data = (uint8_t*)kmalloc(len);
...
struct packet* create_tx_packet(struct nic * n, uint16_t len) {
    ...
    struct packet * p = (struct packet*)kmalloc(sizeof(struct packet));
    p->len = len;
    p->data = (uint8_t*)kmalloc(len);
    memset(p->data, 0, len);
    p->next = 0;
