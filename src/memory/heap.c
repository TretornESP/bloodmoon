...
void * umalloc(struct task* task, uint64_t size) {
    ...
    void * result = _malloc(TO_KERNEL_MAP(task->pd), (struct heap*)task->heap, size);
...
void * ucalloc(struct task* task, uint64_t num, uint64_t size) {
    LOCK_HEAP((struct heap*)task->heap);
    void * result = _calloc(TO_KERNEL_MAP(task->pd), (struct heap*)task->heap, num, size);
    UNLOCK_HEAP((struct heap*)task->heap);
    return result;
}
...
void * urealloc(struct task* task, void* buffer, uint64_t size) {
    LOCK_HEAP((struct heap*)task->heap);
    void * result = _realloc(TO_KERNEL_MAP(task->pd), (struct heap*)task->heap, buffer, size);
    UNLOCK_HEAP((struct heap*)task->heap);
    return result;
}
...
void * ustackalloc(struct task* task, uint64_t length) {
    ...
    void * result = _stackalloc(TO_KERNEL_MAP(task->pd), (struct heap*)task->heap, length);
